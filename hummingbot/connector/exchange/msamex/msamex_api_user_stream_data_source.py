...
class MsamexAPIUserStreamDataSource(UserStreamTrackerDataSource):
    ...
    def __init__(self, msamex_client: MsamexClient, domain: str = "com"):
    ...
    async def get_listen_key(self):
        async with aiohttp.ClientSession() as client:
            url = MSAMEX_API_ENDPOINT.format(self._domain)
            async with client.post(f"{url}{MSAMEX_USER_STREAM_ENDPOINT}",
                                   headers={"X-MBX-APIKEY": self._msamex_client.API_KEY}) as response:
    ...
    async def ping_listen_key(self, listen_key: str) -> bool:
        async with aiohttp.ClientSession() as client:
            url = MSAMEX_API_ENDPOINT.format(self._domain)
            async with client.put(f"{url}{MSAMEX_USER_STREAM_ENDPOINT}",
                                  headers={"X-MBX-APIKEY": self._msamex_client.API_KEY},
    ...
    async def get_ws_connection(self) -> websockets.WebSocketClientProtocol:
        url = MSAMEX_WSS_USER_STREAM.format(self._domain)
