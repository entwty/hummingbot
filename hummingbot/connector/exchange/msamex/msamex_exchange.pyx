...
from msamex import client as msamex_client_module
from msamex.exceptions import MsamexAPIException
...
from .msamex_order_book_tracker import MsamexOrderBookTracker
from .msamex_user_stream_tracker import MsamexUserStreamTracker
from .msamex_time import MsamexTime
from .msamex_in_flight_order import MsamexInFlightOrder
...
cdef class MsamexExchangeTransactionTracker(TransactionTracker):
    cdef:
        MsamexExchange _owner

    def __init__(self, owner: MsamexExchange):
        super().__init__()
        self._owner = owner
...
cdef class MsamexExchange(ExchangeBase):
    ...
    MSAMEX_TRADE_TOPIC_NAME = "msamex-trade.serialized"
    MSAMEX_USER_STREAM_TOPIC_NAME = "msamex-user-stream.serialized"
    ...
    def __init__(self,
                 msamex_api_key: str,
                 msamex_api_secret: str,
                 trading_pairs: Optional[List[str]] = None,
                 trading_required: bool = True,
                 domain="com"
                 ):
        self._domain = domain
        self.monkey_patch_msamex_time()
        super().__init__()
        self._trading_required = trading_required
        self._order_book_tracker = MsamexOrderBookTracker(trading_pairs=trading_pairs, domain=domain)
        self._msamex_client = MsamexClient(msamex_api_key, msamex_api_secret, tld=domain)
        self._user_stream_tracker = MsamexUserStreamTracker(msamex_client=self._msamex_client, domain=domain)
        self._ev_loop = asyncio.get_event_loop()
        self._poll_notifier = asyncio.Event()
        self._last_timestamp = 0
        self._in_flight_orders = {}  # Dict[client_order_id:str, MsamexInFlightOrder]
        self._order_not_found_records = {}  # Dict[client_order_id:str, count:int]
        self._tx_tracker = MsamexExchangeTransactionTracker(self)
    ...
    @property
    def msamex_client(self) -> MsamexClient:
        return self._msamex_client
    ...
    @property
    def order_book_tracker(self) -> MsamexOrderBookTracker:
        return self._order_book_tracker

    @property
    def user_stream_tracker(self) -> MsamexUserStreamTracker:
    ...
    async def get_active_exchange_markets(self) -> pd.DataFrame:
        return await MsamexAPIOrderBookDataSource.get_active_exchange_markets()

    def monkey_patch_msamex_time(self):
        if msamex_client_module.time != MsamexTime.get_instance():
            msamex_client_module.time = MsamexTime.get_instance()
            MsamexTime.get_instance().start()

    async def schedule_async_call(
            self,
            coro: Coroutine,
            timeout_seconds: float,
            app_warning_msg: str = "Msamex API call failed. Check API key and network connection.") -> any:
    ...
    async def query_api(
            ...
            func,
            *args,
            app_warning_msg: str = "Msamex API call failed. Check API key and network connection.",
        ...
        async with self._throttler.weighted_task(request_weight=request_weight):
            try:
                ...
                if "Timestamp for this request" in str(ex):
                    self.logger().warning("Got Msamex timestamp error. "
                                          "Going to force update Msamex server time offset...")
                    msamex_time = MsamexTime.get_instance()
                    msamex_time.clear_time_offset_ms_samples()
                    await msamex_time.schedule_update_server_time_offset()
                raise ex
    ...
    async def _update_balances(self):
        ...
        account_info = await self.query_api(self._msamex_client.get_account)
    ...
    async def _update_trade_fees(self):
        ...
        if current_timestamp - self._last_update_trade_fees_timestamp > 60.0 * 60.0 or len(self._trade_fees) < 1:
            try:
                res = await self.query_api(self._msamex_client.get_trade_fee)
                ...
                self.logger().network("Error fetching Msamex trade fees.", exc_info=True,
                                      app_warning_msg=f"Could not fetch Msamex trading fees. "
    ...
    cdef object c_get_fee(self,
        ...
        if order_type.is_limit_type() and fee_overrides_config_map["msamex_maker_fee"].value is not None:
            return TradeFee(percent=fee_overrides_config_map["msamex_maker_fee"].value / Decimal("100"))
        if order_type is OrderType.MARKET and fee_overrides_config_map["msamex_taker_fee"].value is not None:
            return TradeFee(percent=fee_overrides_config_map["msamex_taker_fee"].value / Decimal("100"))

        if trading_pair not in self._trade_fees:
            # https://www.msamex.com/en/fee/schedule
    ...
    async def _update_trading_rules(self):
        ...
        if current_tick > last_tick or len(self._trading_rules) < 1:
            exchange_info = await self.query_api(self._msamex_client.get_exchange_info)
    ...
    async def _update_order_fills_from_trades(self):
        cdef:
            ...
            # in case Msamex's user stream events are not working.
            # This is separated from _update_order_status which only updates the order status without producing filled
            # events, since Msamex's get order endpoint does not return trade IDs.
        ...
        if current_tick > last_tick:
            if len(self._in_flight_orders) > 0:
                ...
                tasks = [self.query_api(self._msamex_client.get_my_trades, symbol=convert_to_exchange_trading_pair(trading_pair))
    ...
    async def _update_order_status(self):
        cdef:
            # This is intended to be a backup measure to close straggler orders, in case Msamex's user stream events
            # are not working.
            # The minimum poll interval for order status is 10 seconds.
            int64_t last_tick = <int64_t>(self._last_poll_timestamp / self.UPDATE_ORDER_STATUS_MIN_INTERVAL)
            int64_t current_tick = <int64_t>(self._current_timestamp / self.UPDATE_ORDER_STATUS_MIN_INTERVAL)
        ...
        if current_tick > last_tick and len(self._in_flight_orders) > 0:
            ...
            tasks = [self.query_api(self._msamex_client.get_order,
            ...
            for order_update, tracked_order in zip(results, tracked_orders):
                ...
                if isinstance(order_update, Exception):
                    if order_update.code == 2013 or order_update.message == "Order does not exist.":
                        ...
                        if self._order_not_found_records[client_order_id] < self.ORDER_NOT_EXIST_CONFIRMATION_COUNT:
                            ...
                            # it as failed. See: https://github.com/entwty/hummingbot/issues/601
                            continue
                ...
                order_type = MsamexExchange.to_hb_order_type(order_update["type"])
        ...
        while True:
            try:
                ...
                self.logger().network(
                    ...
                    exc_info=True,
                    app_warning_msg="Could not fetch user events from Msamex. Check API key and network connection."
                )
    ...
    async def _user_stream_event_listener(self):
        async for event_message in self._iter_user_event_queue():
            try:
                ...
                # Refer to https://github.com/msamex-exchange/msamex-official-api-docs/blob/master/user-data-stream.md
                # As per the order update section in Msamex the ID of the order being cancelled is under the "C" key
                if event_type == "executionReport":
                    ...
                    if execution_type == "TRADE":
                        order_filled_event = OrderFilledEvent.order_filled_event_from_msamex_execution_report(event_message)
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching account updates.", exc_info=True,
                                      app_warning_msg="Could not fetch account updates from Msamex. "
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching trading rules.", exc_info=True,
                                      app_warning_msg="Could not fetch new trading rules from Msamex. "
    ...
    async def server_time(self) -> int:
        ...
        """
        result = await self.query_api(self._msamex_client.get_server_time)
    ...
    async def check_network(self) -> NetworkStatus:
        try:
            await self.query_api(self._msamex_client.ping)
        except asyncio.CancelledError:
            raise
        except Exception:
    ...
    @staticmethod
    def msamex_order_type(order_type: OrderType) -> str:
        return order_type.name.upper()

    @staticmethod
    def to_hb_order_type(msamex_type: str) -> OrderType:
        return OrderType[msamex_type]
    ...
    async def create_order(self,
        ...
        price_str = f"{price:f}"
        type_str = MsamexExchange.msamex_order_type(order_type)
        side_str = MsamexClient.SIDE_BUY if trade_type is TradeType.BUY else MsamexClient.SIDE_SELL
        ...
        if order_type == OrderType.LIMIT:
            api_params["timeInForce"] = MsamexClient.TIME_IN_FORCE_GTC
        ...
        try:
            order_result = await self.query_api(self._msamex_client.create_order, **api_params)
            ...
            self.logger().network(
                f"Error submitting {side_str} {type_str} order to Msamex for "
                f"{amount} {trading_pair} "
                f"{price}.",
                exc_info=True,
    ...
    async def execute_cancel(self, trading_pair: str, order_id: str):
        try:
            cancel_result = await self.query_api(self._msamex_client.cancel_order,
                                                 symbol=convert_to_exchange_trading_pair(trading_pair),
                                                 origClientOrderId=order_id)
        except MsamexAPIException as e:
            if "Unknown order sent" in e.message or e.code == 2011:
                # The order was never there to begin with. So cancelling it is a no-op but semantically successful.
                self.logger().debug(f"The order {order_id} does not exist on Msamex. No cancellation needed.")
                self.c_stop_tracking_order(order_id)
                self.c_trigger_event(self.MARKET_ORDER_CANCELLED_EVENT_TAG,
        ...
        try:
            async with timeout(timeout_seconds):
                ...
                for cr in cancellation_results:
                    if isinstance(cr, MsamexAPIException):
                        continue
            ...
            self.logger().network(
                ...
                exc_info=True,
                app_warning_msg="Failed to cancel order with Msamex. Check API key and network connection."
            )
    ...
    async def get_open_orders(self) -> List[OpenOrder]:
        orders = await self.query_api(self._msamex_client.get_open_orders)
        ret_val = []
        for order in orders:
    ...
    async def get_all_my_trades(self, trading_pair: str) -> List[Trade]:
        # Ths Msamex API call rate is 5, so we cache to make sure we don't go over rate limit
        trades = await self.query_api(self._msamex_client.get_my_trades,
                                      symbol=convert_to_exchange_trading_pair(trading_pair))
        from hummingbot.connector.exchange.msamex.msamex_helper import format_trades
    ...
    async def get_my_trades(self, trading_pair: str, days_ago: float) -> List[Trade]:
        ...
        from hummingbot.connector.exchange.msamex.msamex_helper import get_utc_timestamp
