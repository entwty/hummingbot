...
class MsamexAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def fetch_trading_pairs(domain="com") -> List[str]:
        try:
            ...
            # Do nothing if the request fails -- there will be no autocomplete for msamex trading pairs
            pass
            ...
            try:
                ...
                async with websockets.connect(stream_url) as ws:
                    ...
                    async for raw_msg in self._inner_messages(ws):
                        ...
                        trade_msg: OrderBookMessage = MsamexOrderBook.trade_message_from_exchange(msg)
        ...
        while True:
            try:
                ...
                async with websockets.connect(stream_url) as ws:
                    ...
                    async for raw_msg in self._inner_messages(ws):
                        msg = ujson.loads(raw_msg)
                        order_book_message: OrderBookMessage = MsamexOrderBook.diff_message_from_exchange(
                            msg, time.time())
                ...
                async with aiohttp.ClientSession() as client:
                    for trading_pair in self._trading_pairs:
                        try:
                            ...
                            # Be careful not to go above Msamex's API rate limits.
