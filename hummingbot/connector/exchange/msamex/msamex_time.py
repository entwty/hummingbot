...
class MsamexTime:
    """
    Used to monkey patch Msamex client's time module to adjust request timestamp when needed
    """
    MSAMEX_TIME_API = "https://www.msamex.com/api/v2/peatio/public/timestamp"
    NaN = float("nan")
    _bt_logger = None
    ...
    @classmethod
    def get_instance(cls) -> "MsamexTime":
        if cls._bt_shared_instance is None:
            cls._bt_shared_instance = MsamexTime()
    ...
    async def update_server_time_offset(self):
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(self.MSAMEX_TIME_API) as resp:
                    resp_data: Dict[str, float] = await resp.json()
                    msamex_server_time_ms: float = float(resp_data["serverTime"])
                    local_after_ms: float = time.perf_counter() * 1e3
            local_server_time_pre_image_ms: float = (local_before_ms + local_after_ms) / 2.0
            time_offset_ms: float = msamex_server_time_ms - local_server_time_pre_image_ms
            self.add_time_offset_ms_sample(time_offset_ms)
            self._last_update_local_time = time.perf_counter()
        except asyncio.CancelledError:
            raise
        except Exception:
            self.logger().network("Error getting Msamex server time.", exc_info=True,
                                  app_warning_msg="Could not refresh Msamex server time. "
