...
from . import msamex_utils

_bob_logger = None


cdef class MsamexOrderBook(OrderBook):
    @classmethod
    ...
    def diff_message_from_exchange(cls,
        ...
        return OrderBookMessage(OrderBookMessageType.DIFF, {
            "trading_pair": msamex_utils.convert_from_exchange_trading_pair(msg["s"]),
    ...
    def diff_message_from_db(cls, record: RowProxy, metadata: Optional[Dict] = None) -> OrderBookMessage:
        msg = ujson.loads(record["json"])  # Msamex json in DB is TEXT
        if metadata:
            msg.update(metadata)
        return OrderBookMessage(OrderBookMessageType.DIFF, {
            "trading_pair": msamex_utils.convert_from_exchange_trading_pair(msg["s"]),
